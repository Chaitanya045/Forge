...
class RoundedGlassScrollBarRender(ScrollBarRender):
    VERTICAL_THUMB_GLYPH = "│"
    HORIZONTAL_THUMB_GLYPH = "─"
    VERTICAL_THIN_BARS = ["│", "│", "│", "│", "│", "│", "│", " "]
    HORIZONTAL_THIN_BARS = ["─", "─", "─", "─", "─", "─", "─", " "]
    ...
    def render_bar(
        ...
        bars = cls.VERTICAL_THIN_BARS if vertical else cls.HORIZONTAL_THIN_BARS
        bars_count = len(bars)
        ...
        if (
            ...
            or not window_size
            or not virtual_size
            or size == virtual_size
            or virtual_size <= window_size
        ):
            ...
            thumb_segment = Segment(
                (cls.VERTICAL_THUMB_GLYPH if vertical else cls.HORIZONTAL_THUMB_GLYPH)
                * width_thickness,
                thumb_style,
            )
            ...
            if start_index < len(segments):
                bar_character = bars[bars_count - 1 - start_bar]
                if bar_character != " ":
            ...
            if end_index < len(segments):
                bar_character = bars[bars_count - 1 - end_bar]
                if bar_character != " ":
